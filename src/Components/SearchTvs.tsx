...
import { getSearchTvs, IGetTvsResult } from "../api";
...
function SearchTvs({keyword}: {keyword:string}) {
  const navigate = useNavigate();
  // bigMovieMatch 타입 선언 안줘 되지만 타입스크립트 공부를 위해 적용
  const bigMovieMatch: PathMatch<string> | null = useMatch("/tvs/:tvId");
  //console.log(bigMovieMatch);
  const { scrollY } = useScroll();
  const { data, isLoading } = useQuery<IGetTvsResult>(
    ["tvs", "search"],
    () => getSearchTvs(keyword as string)
  );
